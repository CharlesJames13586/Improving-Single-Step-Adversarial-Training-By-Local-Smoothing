...
def main():
    ...
    for label in args.model_labels:
        ...
        if args.discrete:
            test_acc_pgd_rr, _, deltas_pgd_rr = utils.rob_acc_discrete(test_batches, model, eps, pgd_alpha, opt, half_prec, attack_iters, n_restarts)
        else:
            test_acc_pgd_rr, _, deltas_pgd_rr = utils.rob_acc(test_batches, model, eps, pgd_alpha, opt, half_prec, attack_iters, n_restarts)
        print('[{}: test on {:.1f}k points][iter={}] acc_clean {:.2%}, pgd_rr {:.2%}, {:.2f}m'.format(label,len(test_batches)/1000, "???", test_acc_clean, test_acc_pgd_rr, (time.time()-start_time)/60))
